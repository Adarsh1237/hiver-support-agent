...
This repo does **not** commit the real dataset — `data/twcs.csv` (the Kaggle
"Customer Support on Twitter" file) is ~493 MB, too large to keep in Git, so
`data/` only ships the small synthetic offline dataset generated by
`scripts/build_sample_data.py`. If you run any command with a real brand
(e.g. `--brand AmazonHelp`) without first downloading `twcs.csv`, the
pipeline will fail because `load_brand_pairs("AmazonHelp")` needs the real
dataset on disk.

To run the agent against the real dataset:
...
3. Then run:
   ...
   python -m src.pipeline --brand AmazonHelp --message "My order was delayed"
   ```

If you just want to try the agent without downloading anything, use the
offline synthetic dataset instead — see "Reproduce in under 15 minutes"
below, which works fully offline with no external dataset or API key.
